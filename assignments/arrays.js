...
console.log("Car 33 is a " + (Object.values(inventory[32]))[3] + " " + (Object.values(inventory[32]))[1] + " " + (Object.values(inventory[32]))[2] + "." )
...
for(i = 0; i<inventory.length;i++)
{
  ...
  {lastCar.push(inventory[i].car_make , inventory[i].car_model)}
};
...
let carModels = [];
let carModelsSorted = [];
for(i =0; i<inventory.length;i++){carModels.push(inventory[i].car_model)}
...
let BMWAndAudi = [];
for(i = 0; i< inventory.length; i++){
  if(inventory[i].car_make ==="BMW" || inventory[i].car_make === "Audi"){BMWAndAudi.push(inventory[i].car_make)}
}
